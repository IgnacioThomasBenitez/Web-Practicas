...
async function cargarAlumnos(){
  ...
  try{
    ...
    const request = await fetch('api/generarAlumnos73', {
      method: 'GET',
      headers: getHeaders()
    });
  ...
  async function eliminarAlumno(id_alumno){
    ...
    const request = await fetch('api/generarAlumnos73/' + id_alumno, {
      method: 'DELETE',
        headers:getHeaders()
    });
...
async function editarAlumno(id_alumno, id_fila){
  ...
  try{
    ...
    if(datos.Presente == "Presente" && datos.hora == 0){
      ...
      console.log("esta presente pero no tiene hora");
      return;
    ...
    if(datos.Presente == "Presente" && datos.hora >= 0 ){
      ...
      console.log("esta presente y tiene horas");
      editAlumno.faltas = alumnos.faltas
      console.log("ausente=  "+ editAlumno.faltas);
